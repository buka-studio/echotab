  ...
  function BentoCard({ className, illustration, children, ...props }, ref) {
    ...
    return (
      <div className={cn("shadow-card relative rounded-[22px] p-[1px]", className)}>
        <div
          className={cn(
            "relative-mouse [&_>*]:opacity-1 bg-background border-border relative z-[1] flex h-[414px] flex-col gap-10 overflow-hidden rounded-2xl border p-6",
          )}
          ref={ref}
          {...props}>
        ...
        <div
          ref={relativeRef}
          className={cn(
            "mouse-border bg-foreground absolute inset-0 rounded-[17px] opacity-30 transition-opacity duration-500 [&]:[mask-image:radial-gradient(40%_40%_at_var(--mouse-x,9999px)_var(--mouse-y,9999px),black_45%,transparent)]",
          )}
        />
      </div>
    );

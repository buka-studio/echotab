...
function FallbackState({
  ...
  return (
    <motion.div
      ...
      {...props}>
      <div
        className="bg-background absolute inset-0 rounded-lg mask-[linear-gradient(0deg,transparent,black)]"
...
export function TabInfoPreview({
  ...
  return (
    <Tabs
      ...
      key={tab.url}>
      <TabsList className="relative h-auto w-full rounded-md">
        <InfoTabTrigger value="image">
          {activeTab === "image" && <TabIndicator />}
          <span className="relative z-10">Image</span>
        </InfoTabTrigger>
        <InfoTabTrigger value="info">
          {activeTab === "info" && <TabIndicator />}
